...
# 加入防呆提示，提醒使用者按 Enter
rename_prefix = st.sidebar.text_input("照片批次命名開頭", value="台灣房屋_捷運樂善物件", help="修改名稱後，請按一下鍵盤的 Enter 鍵！")
...
if uploaded_photos:
    ...
    if st.button("🚀 確認版型！一鍵產出所有照片 (FB畫質最佳化)", type="primary", use_container_width=True):
        ...
        # 🌟 核心修正：強制把「當下的檔名」鎖進記憶體
        safe_name = rename_prefix if rename_prefix else "照片包"
        st.session_state.ready_zip = zip_io.getvalue()
        st.session_state.ready_zip_name = f"{safe_name}.zip"

    # 🌟 核心修正：下載按鈕獨立抓取記憶體中的檔案與名稱
    if st.session_state.ready_zip:
        st.success(f"🎉 處理完成！準備下載：{st.session_state.ready_zip_name}")
        st.download_button(
            label="📥 下載處理完成的照片包 (ZIP)", 
